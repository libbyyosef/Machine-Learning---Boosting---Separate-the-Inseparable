...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        identity_matrix = np.eye(X.shape[1] + int(self.include_intercept_))
        if self.include_intercept_:
            X = np.c_[np.ones(len(X)), X]
            identity_matrix[0, 0] = 0
        avg_xt = X.T / len(X)
        regularization_term = self.lam_ + 1e-12
        coefficient_matrix = avg_xt @ X + regularization_term * identity_matrix
        target_vector = avg_xt @ y
        self.coefs_ = np.linalg.solve(coefficient_matrix, target_vector)
